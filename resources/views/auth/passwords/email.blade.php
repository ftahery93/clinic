        ...
        <div class="center-block w-xxl w-auto-xs p-y-md">
            ...
            <div class="p-a-md box-color r box-shadow-z1 text-color m-a">
                ...
                <form name="reset" method="POST" action="{{ url('/password/email') }}">
                    ...
                    @endif
                    <button type="submit"
                        class="btn primary btn-block p-x-md">{{ trans('backLang.sendPasswordResetLink') }}</button>
                </form>
            </div>
            <p id="alerts-container"></p>
            <div class="p-v-lg text-center">{{ trans('backLang.returnTo') }} <a href="{{ url('/login') }}"
                    class="text-primary _600">{{ trans('backLang.signIn') }}</a></div>
        </div>
        <!-- ############ LAYOUT END-->
    </div>

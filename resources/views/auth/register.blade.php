    ...
    <div class="center-block w-xxl w-auto-xs p-y-md">
        ...
        <div class="p-a-md box-color r box-shadow-z1 text-color m-a">
            ...
            <form role="form" method="POST" action="{{ route('register') }}">
                ...
                <button type="submit" class="btn primary btn-block p-x-md"><i class="material-icons">&#xe7fe;</i> {{ trans('backLang.createNewAccount') }}</button>
            </form>
        </div>
        <div class="p-v-lg text-center">
            <div>{{ trans('backLang.signedInToControl') }} <a href="{{ url('/login') }}" class="text-primary _600">{{ trans('backLang.signIn') }}</a>
            </div>
        </div>
    </div>
    <!-- ############ LAYOUT END-->

    ...
    <div class="center-block w-xxl w-auto-xs p-y-md">
        ...
        <div class="p-a-md box-color r box-shadow-z1 text-color m-a">
            ...
            <form name="form" method="POST" action="{{ url('/login') }}">
                ...
                <div class="m-b-md">
                    <label class="md-check">
                        <input type="checkbox" name="remember"><i class="primary"></i> {{ trans('backend.keepMeSignedIn') }}
                    </label>
                </div>
            ...
            @if(Helper::GeneralWebmasterSettings("register_status"))
                ...
                <div class="text-center">
                    <a href="{{ url('/register') }}" class="btn info btn-block p-x-md"><i class="material-icons">&#xe7fe;</i> {{ trans('backend.createNewAccount') }}
                    </a>
                </div>
            @endif
        </div>
        <div class="p-v-lg text-center">
            <div class="m-b"><a href="{{ url('/password/reset') }}"
                                class="text-primary _600">{{ trans('backend.forgotPassword') }}</a></div>
        </div>
    </div>
    <!-- ############ LAYOUT END-->
</div>

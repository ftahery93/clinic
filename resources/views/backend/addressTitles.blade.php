    ...
    <div class="padding">
        <div class="box">
            <div class="box-header dker">
                <h3>{{ trans('backend.address_titles') }}</h3>
                <small>
                    <a href="{{ route('adminHome') }}">{{ trans('backend.home') }}</a> /
                    <a href="">{{ trans('backend.address_titles') }}</a>
                </small>
            </div>
            @if($AddressTitles->total() > 0)
                @if(@Auth::user()->permissionsGroup->add_status)
                    <div class="row p-a">
                        <div class="col-sm-12">
                                <a class="btn btn-fw primary marginBottom5" href="{{route("addressTitle_create")}}">
            ...
            @endif
            @if($AddressTitles->total() > 0)
                {{Form::open(['route'=> 'addressTitle_update_all','method'=>'post'])}}
                <div class="table-responsive">
                    <table class="table table-striped  b-t">
                        ...
                        <tbody>
                        @foreach($AddressTitles as $AddressTitle)
                            <tr>
                                <td><label class="ui-check m-a-0">
                                        <input type="checkbox" name="ids[]" value="{{ $AddressTitle->id }}"><i
                                                class="dark-white"></i>
                                        {!! Form::hidden('row_ids[]',$AddressTitle->id, array('class' => 'form-control row_no')) !!}
                                    </label>
                                </td>
                                <td>{!! $AddressTitle->name_en   !!}</td>
                               
                                <td class="text-center">
                                    @if(@Auth::user()->permissionsGroup->edit_status)
                                        <a class="btn btn-sm success"
                                           href="{{ route("addressTitle_edit",["id"=>$AddressTitle->id]) }}">
                ...
                <footer class="dker p-a">
                    <div class="row">
                        ...
                        <div class="col-sm-3 text-center">
                            <small class="text-muted inline m-t-sm m-b-sm">{{ trans('backend.showing') }} {{ $AddressTitles->firstItem() }}
                                -{{ $AddressTitles->lastItem() }} {{ trans('backend.of') }}
                                <strong>{{ $AddressTitles->total()  }}</strong> {{ trans('backend.records') }}</small>
                        </div>
